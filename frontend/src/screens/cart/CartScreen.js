      ...
      <Row>
            ...
            cartItems.map((item) => {
              return (
                <Row key={item.product} className="cart__info-item">
                  ...
                  <Col md={5} sm={5} xs={5}>
                    <Link to={`/products/${item.product}`} className="cart__info-product-name">{item.name}</Link>
                  </Col>
                  <Col md={3} sm={3} xs={3}>
                    ${item.price}
                  </Col>
        ...
        <Col md={4}>
          <Card className="cart__purchase-info">
            ...
            <button className="btn btn-cart" onClick={checkoutHandler} disabled={cartItems.length === 0}>Proceed to Checkout</button>
          </Card>
        </Col>
      </Row>
    </section>
