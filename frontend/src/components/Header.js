        ...
        <Navbar.Collapse id="responsive-navbar-nav">
          ...
          <Nav className="ml-auto">
            ...
            {userInfo ? (
              <>
                ...
                <Nav.Link className="nav-link" onClick={logoutHandler}>Logout</Nav.Link>
              </>
            ) : (
              <>
          ...
          {userInfo && userInfo.isAdmin && (
            <>
              ...
              {/* Mobile view */}
              <NavLink to="/admin/userlist" className="nav-link mobile-view">Users</NavLink>
              <NavLink to="/admin/productlist" className="nav-link mobile-view">Products</NavLink>
              <NavLink to="/admin/orderlist" className="nav-link mobile-view">Orders</NavLink>
            </>
          )}
          {userInfo && (
            <Link to="/cart">
              <i className="navbar__icon fa fa-shopping-cart" aria-hidden="true">
                {cartItems.length > 0 && <Badge variant="danger" className="cart-badge">{cartItems.reduce((acc, curr) => acc + curr.quantity, 0)}</Badge>}
              </i>
            </Link>
          )}
        </Navbar.Collapse>
